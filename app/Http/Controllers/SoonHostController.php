...
class SoonHostController extends Controller {
	...
	public function host(Request $request, $page = 0) {
		...
		$images = $request->session()->get('hostImages', function () {
			$findImgs = array();
			$hosts = StoreHostInfo::soonHost(Carbon::now())->select(['store_id', 'host_id', 'store_name', 'host_name'])->distinct()->get();
		...
		return view('soon_host', compact('partImgs', 'pgNavi', 'mst'));
	}

	public function findHost(Request $request) {
		// 即ホスト
		$hosts = StoreHostInfo::soonHost(Carbon::now());

		// 料金・年齢範囲
		$priceRange = explode(';', $request->input('price_range'));
		$ageRange = explode(';', $request->input('age_range'));

		$hosts = $hosts->priceRange($priceRange)->ageRange($ageRange);
		...
		return redirect('/soon_host/1')->withInput();
	}
